    ...
    <body class="min-h-screen bg-stone-50 text-stone-950 dark:bg-stone-950 dark:text-stone-50">
        <main class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div class="mt-5 flex gap-4 overflow-x-auto pb-3">
                    @forelse ($boats as $boat)
                        <div class="flex min-w-32 shrink-0 flex-col items-center gap-3 border border-stone-200 bg-white px-5 py-5 shadow-sm dark:border-stone-800 dark:bg-stone-900">
                            ...
                            @endif
                            <form class="reorganization-action hidden" method="POST" action="{{ route('admin.boats.move-outside', $boat) }}">
                                @csrf
                                <button type="submit" class="cursor-pointer border border-rose-600 px-3 py-2 text-xs font-semibold text-rose-700 transition hover:bg-rose-50 focus:outline-none focus:ring-2 focus:ring-rose-500 dark:text-rose-400 dark:hover:bg-rose-950">Sortir</button>
                            </form>
                        </div>
                    @empty
            ...
            <section class="mt-12 border-t border-stone-200 pt-8 dark:border-stone-800">
                ...
                <ul class="mt-5 divide-y divide-stone-200 border-y border-stone-200 dark:divide-stone-800 dark:border-stone-800">
                    @forelse ($boatsOutsideWarehouse as $boat)
                        <li class="px-4 py-4">
                            ...
                            <form class="reorganization-action hidden mt-3" method="POST" action="{{ route('admin.boats.move-inside', $boat) }}">
                                @csrf
                                <button type="submit" class="cursor-pointer border border-emerald-600 px-3 py-2 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:text-emerald-400 dark:hover:bg-emerald-950">Rentrer</button>
                            </form>
                        </li>
                    @empty
                ...
                <button id="reorganize-boats" type="button" class="mt-6 cursor-pointer border border-sky-600 bg-sky-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 dark:ring-offset-stone-950">
                    Réorganiser les bateaux
                </button>
            </section>
        </main>
        <script>
            document.getElementById('reorganize-boats').addEventListener('click', function () {
                document.querySelectorAll('.reorganization-action').forEach(function (action) {
