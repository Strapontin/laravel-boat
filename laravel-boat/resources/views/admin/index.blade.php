...
<body class="min-h-screen bg-stone-50 text-stone-950 dark:bg-stone-950 dark:text-stone-50">
    @php($isReorganizing = request()->boolean('reorganize'))
    <main class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section>
            ...
            <div class="mt-5 flex gap-4 overflow-x-auto pb-3">
                ...
                <div
                    class="flex min-h-64 min-w-32 shrink-0 flex-col items-center gap-3 border border-stone-200 bg-white px-5 py-5 shadow-sm dark:border-stone-800 dark:bg-stone-900">
                    ...
                    @endif
                    <form
                        class="reorganization-action mt-auto flex w-full justify-center{{ $isReorganizing ? '' : ' hidden' }}"
                        method="POST" action="{{ route('admin.boats.move-outside', $boat) }}">
                        @csrf
                        <button type="submit"
        ...
        <section class="mt-12 border-t border-stone-200 pt-8 dark:border-stone-800">
            ...
            <div class="mt-5 flex gap-4 overflow-x-auto pb-3">
                ...
                <div
                    ...
                    <form
                        class="reorganization-action mt-auto flex w-full justify-center{{ $isReorganizing ? '' : ' hidden' }}"
                        method="POST" action="{{ route('admin.boats.move-inside', $boat) }}">
                        @csrf
                        <button type="submit"
                ...
                @empty
                <p class="text-sm text-stone-500 dark:text-stone-400">Aucun bateau en dehors de l'entrepôt.</p>
                @endforelse
            </div>
